...
public class Detailfactory {
	 ...
	 */
	public static View getView(JSONObject leg, Context context)
			throws JSONException {
		...
		String className = Detailfactory.class.getPackage().getName() + "."
				+ mode + "DetailGenerator"; // get the current package name?
		...
		return getDetailGeneratorInstance(view, context, leg, detailGenerator)
				.getView();
	}
	...
	private static int getRecourceId(String viewName, Context context) {
		...
		int resource = context.getResources().getIdentifier(
				VIEW_PREFIX + viewName, "layout", packageName);
		return resource != 0 ? resource : context.getResources().getIdentifier(
				VIEW_DEFAULT, "layout", packageName);
	}

	private static BaseDetailGenerator getDetailGeneratorInstance(View view,
			Context context, JSONObject leg, Class<?> detailGenerator)
			throws JSONException {
		try {
			Constructor<?> constructor = detailGenerator.getConstructor(
					View.class, Context.class, JSONObject.class);
			return (BaseDetailGenerator) constructor.newInstance(view, context,
					leg);
		} catch (Exception e) {
